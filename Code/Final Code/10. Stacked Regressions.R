...
# save different fixed effect possibilities
long_fes <- c("firm^dataset", "state_year^dataset", "industry_year^dataset")
long_fes_ff <-  c("firm^dataset", "state_year^dataset", "industry_ff_year^dataset")
short_fes <- c("firm^dataset", "year^dataset")
...
# function to make stacked dataset
make_stacked <- function(dt, law) {
  ...
  # keep only the years with greater than or equal to 10 firms
  ...
  stack <- function(tyr) {
    ...
    controls <- dt %>% 
      # not treated in that year or before
...
# make code to run the event studies
...
# save
ggsave(bc_stacked, filename = paste(dropbox, "figures/bc_stacked.pdf", sep = ""), dpi = 500,
       width = 7.5, height = 9)
...
# save
ggsave(pp_stacked, filename = paste(dropbox, "figures/pp_stacked.pdf", sep = ""), dpi = 500,
       width = 7.5, height = 9)
...
# save
ggsave(figB3, filename = paste(dropbox, "figures/figB3.pdf", sep = ""), dpi = 500,
       width = 7.5, height = 9)
...
# save
ggsave(figB4, filename = paste(dropbox, "figures/figB4.pdf", sep = ""), dpi = 500,
       width = 7.5, height = 9)