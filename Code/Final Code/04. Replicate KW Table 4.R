...
# function to get statistical significance markers
...
runmod <- function(depvar, covs) {
  ...
  feols(.[depvar] ~ .[covs] | firm + state_year + industry_year, data = KW, cluster = ~incorporation)
}
...
# fix up some weird naming issues manually to match exactly KW Table IV as published
tbl <- tbl %>% 
...
KW_table <- tbl %>% 
  kable(format = "latex", digits = 3, booktabs = T, align = 'l',
        label = "kw_table", caption = "Replication of Table IV from Karpoff-Wittry (2018)",
...
# save the table
write_lines(KW_table, file = paste(dropbox, "tables/KW_Table.tex", sep = ""))