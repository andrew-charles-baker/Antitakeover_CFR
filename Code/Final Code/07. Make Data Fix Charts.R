...
# function to get significance indicators
...
# two different fes
fe1 <- c("firm", "state_year","industry_year")
fe2 <- c("firm", "state_year", "industry_ff_year")

# function to run models based on dep variable and covariate set
runmod <- function(depvar, covs, dt, fes, modname, dataname) {
  
  # estimate the model
  mod <- feols(.[depvar] ~ .[covs] | .[fes], data = dt, cluster = ~incorporation)
  
  # get data from model into a dataset
...
### Make comparison table - keep only leverage and make table
data_fix <- all_models_data %>% 
  filter(var == "leverage") %>% 
  ...
  kable(format = "latex", digits = 3, booktabs = T, align = 'lcccccccc',
        label = "data_fix", caption = "Comparison of DiD Estimates on Leverage Across Data and Models",
        col.names = linebreak(c(" ", rep(paste0("(", 1:4, ")"), 2)), align = 'c'),
        linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position"), 
                font_size = 8, full_width = T) %>% 
  column_spec(1, width = "4cm") %>% 
  add_header_above(c(" " = 1, "Short Regression Model" = 4, "Long Regression Model" = 4)) %>% 
  add_header_above(c(" " = 1, "Leverage" = 8))

# save the comparison tables
write_lines(data_fix, file = paste(dropbox, "tables/data_fix.tex", sep = ""))
...
figa <- all_models_data %>%
  ...
  geom_pointrange(aes(x = name, y = estimate, ymin = conf.low, ymax = conf.high),
                  color = "#767676FF", size = 0.5) + 
  geom_hline(yintercept = 0, color = "#800000FF", linetype = "longdash", size = 1) + 
  labs(x = "Dataset", y = "") + 
  coord_flip() + 
...
fig <- figa + figb + plot_layout(nrow = 2)

# save the figure
ggsave(fig, filename = paste(dropbox, "figures/datafix_compare.pdf", sep = ""), dpi = 500,
       width = 12, height = 6)

# Make tables for Data 2-4 for the appendix
# function to run models based on dep variable and covariate set
runmod <- function(depvar, covs, dt, fe) {
  feols(.[depvar] ~ .[covs] | .[fe], data = dt, cluster = ~incorporation)
}
...
# fix up some weird naming issues manually to match KW table IV
tbl <- tbl %>% 
  mutate(name = c("Business combination", "law (BC)", "First-generation law", "", "Poison Pill law (PP)", 
                  "", "Control share", "acquisition law (CS)", "Directors' duties law", 
                  "(DD)", "Fair price law (FP)", "", "CS x CTS", "", "BC x Amanda", "", 
...
# save
write_lines(tableA2, file = paste(dropbox, "tables/tableA2.tex", sep = ""))
...
# save
write_lines(tableA3, file = paste(dropbox, "tables/tableA3.tex", sep = ""))
...
# save
write_lines(tableA4, file = paste(dropbox, "tables/tableA4.tex", sep = ""))
