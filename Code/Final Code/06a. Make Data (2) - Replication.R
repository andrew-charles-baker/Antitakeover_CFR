...
# drop if missing headquarter state or industry
comp <- comp %>%
  filter(!is.na(sic) & !is.na(state))

# * Generate industry- and state-year indicators
comp <- comp %>% 
...
# * Generate the number of years a company has been in compustat for age control age = log(1+time in compustat)
comp <- comp %>% 
  group_by(gvkey) %>% 
...
#   * Legal cases
# *************************************************************************************************************************
# make variables for the long regression models
comp <- comp %>% 
...
# the ISS data only comes out for certain years and the literature essentially 
...
# replace opt outs with 0 following KW
comp <- comp %>% 
