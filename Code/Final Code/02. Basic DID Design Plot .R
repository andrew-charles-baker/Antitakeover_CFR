...
plot1 <- data %>% 
  ggplot(aes(x = Time, y = Count, group = Circuit, color = Circuit)) + 
  geom_point() + 
  geom_line() + 
  geom_label(aes(label = Count), hjust = 0.5, vjust = -0.5, color = "black") + 
...
data_fd <- data %>% 
  group_by(Circuit) %>% 
  mutate(Count = Count - Count[which(Time == 0)])
...
plot2 <- data %>% 
  ...
  geom_point(data = data_fd, aes(x = Time, y = Count, group = Circuit, color = Circuit)) +
  geom_line(data = data_fd, aes(x = Time, y = Count, group = Circuit, color = Circuit)) +
...
basic_did <- plot1 + plot2 + plot3

ggsave(basic_did, filename = paste(dropbox, "figures/basic_did.pdf", sep = ""), dpi = 500,
       width = 12, height = 5)